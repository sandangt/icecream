...
import static sanlab.icecream.echo.exception.EchoErrorModel.SERVICE_SUCCESSFUL_PAYMENT_PAYLOAD_INVALID;
...
public class PaymentNotificationService {
    ...
    private void sendSuccessfulPaymentEmail(PaymentNotificationDto payload) {
        ...
        if (payloadOptional.map(PaymentNotificationDto::getEmail).isEmpty()) {
            throw new IcRuntimeException(SERVICE_SUCCESSFUL_PAYMENT_PAYLOAD_INVALID, "Empty email");
        }
        ...
        payloadOptional.map(PaymentNotificationDto::getTime).ifPresent(inner ->
            args.put("time", DatetimeUtils.toFormalStringUTC(inner, DatetimeUtils.PatternType.TYPE_1))
        );
        ...
        try {
            emailRepository.sendToSingle(payload.getEmail(), "Payment Confirmation", "email/successful-payment", args);
        } catch (MessagingException ex) {
            LogUtils.logException(log, ex);
            throw new IcRuntimeException(MAIL_SEND_EMAIL_FAILED, "Failed to send email for successful payment message");
        }
    }
